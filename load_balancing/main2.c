...
int main( int argc, char** argv ) {
	...
	int q_balancer_server_id = msgget( key, IPC_CREAT | 0664 ) ;
	...
	if ( pid == 0 ) {
		...
		for ( k = 0 ; k < 6 ; k++ ) {
			...
			printf("[BALANCER %d] Ho ricevuto un messaggio, inoltro a Server\n",k) ;

			m2.type = k%3 ;
			m2.pid = m1.pid ;

			msgsnd( q_balancer_server_id, (void*) &m2, sizeof(Messaggio)-sizeof(long), 0) ;
	...
	for ( i = 0 ; i < 3 ; i++ ) {
		...
		if ( pid == 0 ) {
			...
			for ( j = 0 ; j < 2 ; j++ ) {
				...
				Messaggio m3 ;
				int ret_val = msgrcv( q_balancer_server_id, (void*) &m3, sizeof(Messaggio)-sizeof(long), i , 0) ;
	...
	msgctl( q_balancer_server_id, IPC_RMID, NULL ) ;
